...
from src.Module import ImageDecoder, ImageDecoder_Start, ImageDecoder_End
from sam2.build_sam import build_sam2_video_predictor, build_sam2

def export_image_encoder(model,onnx_path):
    input_img = torch.randn(1, 3,1024, 1024).cpu()
    #batch_size = torch.randint(1, 4, (1,), dtype=torch.int32)  # 生成一个形状为(1,)的整数张量
    batch_size = torch.tensor([2, 2, 2, 2, 2], dtype=torch.int32) #shape(5,)
...
def export_memory_attention5_5(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention6_6(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
    memory_0 = torch.randn(4,1,256)
    memory_1 = torch.randn(4,1,64,64,64)
...
def export_memory_attention7_7(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention7_8(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention7_9(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention7_10(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention7_11(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention7_12(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention7_13(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention7_14(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention7_15(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_memory_attention7_16(model,onnx_path):
    current_vision_feat = torch.randn(1,256,64,64)        #[1, 256, 64, 64],当前帧的视觉特征
    current_vision_pos_embed = torch.randn(4096,1,256)    #[4096, 1, 256],当前帧的位置特征
...
def export_image_decoderStart(model,onnx_path):
    point_coords = torch.randint(0, 1024, (3, 2, 2), dtype=torch.float)

    point_labels = torch.tensor([[-1, -1], [-1, -1], [-1, -1]],dtype=torch.int32)
    image_embed = torch.randn(3,256,64,64).cpu()
    high_res_feats_0 = torch.randn(3,32,256,256).cpu()
    high_res_feats_1 = torch.randn(3,64,128,128).cpu()
...
def export_image_decoderEnd(model,onnx_path):
    object_score_logits = torch.randn(3,1).cpu()
    frame_size = torch.tensor([1080, 1920],dtype=torch.int32)
    low_res_masks = torch.randn(3,1,256,256).cpu()
    high_res_masks = torch.randn(3,1,1024,1024).cpu()
    sam_output_token = torch.randn(3,256).cpu()
    out = model(
...
def export_memory_encoder(model,onnx_path):
    mask_for_mem = torch.randn(10,1,1024,1024)    #[batch_size, 1, 1024, 1024]
    pix_feat = torch.randn(10,256,64,64)
...
#****************************************************************************
model_type = ["tiny","small","large","base_plus"][3]
...
if __name__ == "__main__":
    ...
    # export_memory_attention6_6(mem_attention,args.outdir)


    # image_decoderStart = ImageDecoder_Start(sam2_model).cpu()
    # export_image_decoderStart(image_decoderStart,args.outdir)


    # image_decoderEnd = ImageDecoder_End(sam2_model).cpu()
    # export_image_decoderEnd(image_decoderEnd,"./")
