...
class ImageEncoder(nn.Module):
    ...
    def forward(self,
                image: torch.Tensor,
                batch_size: torch.Tensor) ->tuple[torch.Tensor, torch.Tensor, torch.Tensor,torch.Tensor,torch.Tensor]:
        ...
        for i, feat in enumerate(expanded_backbone_out["backbone_fpn"]):
            expanded_backbone_out["backbone_fpn"][i] = feat.expand(batch_size.size(0), -1, -1, -1)
        for i, pos in enumerate(expanded_backbone_out["vision_pos_enc"]):
            expanded_backbone_out["vision_pos_enc"][i] = pos.expand(batch_size.size(0), -1, -1, -1)
        ...
        current_vision_feat2 = current_vision_feat.reshape(64,64, batch_size.size(0),256).permute(2, 3, 0, 1) # [1,256,64,64]

        pix_feat = current_vision_feats[-1].permute(1, 2, 0).view(batch_size.size(0), 256, 64, 64)

        # flatten HWxNxC -> NxCxHxW
        high_res_features_0 = current_vision_feats[0].reshape(256,256, batch_size.size(0), 32).permute(2, 3, 0, 1) # [1, 32, 256, 256]
        high_res_features_1 = current_vision_feats[1].reshape(128,128, batch_size.size(0), 64).permute(2, 3, 0, 1) # [1, 64, 128, 128]
