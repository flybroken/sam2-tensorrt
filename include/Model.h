...
#define USE_GPU_ZERO_COPY


struct sKfOut {
    std::vector<float> best_iou;
    float obj_score;
    std::vector<Ort::Value> low_res_masks;
    std::vector<Ort::Value> high_res_masks;
    std::vector<Ort::Value> sam_output_token;
    std::vector<Ort::Value> object_score_logits;
};
...
public:
    virtual ~Model(){};
    virtual bool inference(cv::Mat &image) = 0;
    virtual bool initialize(std::vector<std::string>& tensorrt_paths, int trackType, int gpuId) = 0;
protected:
